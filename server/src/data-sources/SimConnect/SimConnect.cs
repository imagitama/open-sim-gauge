...
public class SimConnectDataSource : DataSourceBase
{
    ...
    private readonly Dictionary<(string VarName, string Unit), List<Action<object>>> _varCallbacks = new();
    ...
    public override async Task SubscribeToVar(string varName, string unit, Action<object> callback)
    {
        ...
        if (!GetIsSubscribed(varName, unit))
        {
            ...
            if (!_varCallbacks.ContainsKey(key))
            {
                _varCallbacks[key] = new List<Action<object>>();
            }

            _varCallbacks[key].Add(callback);
    ...
    public override async Task Listen()
    {
        _ = Task.Run(async () =>
        {
            Console.WriteLine($"[SimConnect] Listening at rate {_config.Rate}ms");
            ...
            while (IsConnected)
            {
                ...
                {
                    Console.WriteLine($"[SimConnect] Receive message error: {ex.Message}");
                }
